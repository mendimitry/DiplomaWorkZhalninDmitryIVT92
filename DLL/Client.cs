...
namespace NetworkDiscovery
{
    ...
    public class Client : IDisposable
    {
        ...
        private String GetCheckedValue(NameValueCollection values, String key, Dictionary<String, String> defaultConfigValues)
        {
            ...
            else
            {
                if (!String.IsNullOrEmpty(key) && !values.AllKeys.Contains(key))
                {
                    ...
                    {
                        MessageBox.Show($"CLIENT: Warning, key <{key}> is not defined in App.config and will be initialized with default value {defaultConfigValues[key]}");
                        return defValueOutput;
                    }
                    else
                    {
                        MessageBox.Show($"CLIENT: Error, specific key <{key}> is not defined in App.config");
                    }
                }
                return null;
            }
        ...
        public void Init()
        {
            ...
            if (_endPoint == null)
                MessageBox.Show($"CLIENT: Error, failed on initialization <_endPoint>");
